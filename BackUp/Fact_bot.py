...
async def start_bot(message: types.Message):
    db.del_info(message.from_user.id)
...
async def start_bot2(message:types.Message):
    ...
    result = db.show_all(message.from_user.id)

    if result == []:
        await message.answer(f'Вы нечего не вводили, история пустая',reply_markup=kb3)
    else:
        for i in result:
          await message.answer(f'Текст: {i[0]}, Тип текста: {i[1]}', reply_markup=kb4)
...
async  def state_1(message: types.Message, state: FSMContext):
    ...
    if not num.isdigit():
        ...
        db.add_info([int(message.from_user.id), message.from_user.first_name, result['text'], result["type"]])
